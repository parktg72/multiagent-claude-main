...
python3 -B -c 'import ast, pathlib, sys; [ast.parse(pathlib.Path(p).read_text(encoding="utf-8"), filename=p) for p in sys.argv[1:]]' "$ROOT/bin/worker.py" "$ROOT/tests/test_worker.py" "$ROOT/tests/test_hardening.py"
